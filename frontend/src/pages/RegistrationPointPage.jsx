...
import {
  ...
  Chip, Stack, Avatar, Tooltip, TextField, InputAdornment, LinearProgress,
  Skeleton, Divider,
} from "@mui/material";
...
export default function RegistrationPointPage() {
  ...
  if (loading) {
    return (
      <Box sx={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #fff8e1 0%, #fffde7 100%)" }}>
        <Stack spacing={2} alignItems="center">
          <LinearProgress sx={{ width: 240 }} />
          <Typography color="text.secondary">กำลังโหลดข้อมูลระบบ...</Typography>
        </Stack>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ minHeight: "100vh", pb: 6, background: "radial-gradient(1200px 600px at 20% -10%, #fff7db 0%, transparent 60%), radial-gradient(1200px 600px at 120% 110%, #ffe082 0%, transparent 60%), linear-gradient(135deg,#fff8e1 0%,#fffde7 100%)" }}>
      <Box sx={{ maxWidth: 1080, mx: "auto", px: 2 }}>
        <Card
          elevation={headerHover ? 8 : 4}
          onMouseEnter={() => setHeaderHover(true)}
          onMouseLeave={() => setHeaderHover(false)}
          sx={{ mt: 4, borderRadius: 4, background: "linear-gradient(90deg, rgba(255,243,224,.9), rgba(255,224,130,.9))", boxShadow: "0 10px 30px rgba(255,193,7,0.25), inset 0 1px 0 rgba(255,255,255,.6)", border: "1px solid rgba(255,193,7,.35)" }}
        >
          <CardContent>
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center" justifyContent="space-between">
              <Stack direction="row" spacing={2} alignItems="center">
                <Avatar src="/logo.svg" sx={{ width: 56, height: 56, bgcolor: "#fff", boxShadow: "0 6px 18px rgba(255,193,7,.45)", border: "2px solid rgba(255,193,7,.7)" }} variant="circular" />
                <Box>
                  <Typography variant="h5" fontWeight={900} color="primary" sx={{ letterSpacing: 0.5 }}>จัดการจุดลงทะเบียน</Typography>
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Chip label="Admin Only" size="small" color="warning" sx={{ fontWeight: 800 }} />
                    {lastFetch && <Typography variant="body2" color="text.secondary">อัปเดตล่าสุด: {sinceText}</Typography>}
                  </Stack>
                </Box>
              </Stack>
              <Stack direction="row" spacing={1.2} alignItems="center">
                ...
                <Tooltip title="รีเฟรชข้อมูล">
                  <span>
                    <IconButton onClick={fetchData} disabled={fetching} color="primary" sx={{ bgcolor: "#fff", border: "1px solid rgba(0,0,0,.06)", "&:hover": { bgcolor: "#fffbe6" } }}><RefreshIcon /></IconButton>
                  </span>
                </Tooltip>
                <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={handleOpenAdd} sx={{ fontWeight: 800, borderRadius: 2.5, boxShadow: "0 6px 18px rgba(255,193,7,.35)" }}>
                  เพิ่มจุดลงทะเบียน
                </Button>
              </Stack>
            </Stack>
            ...
            <TextField
              ...
              InputProps={{ startAdornment: (<InputAdornment position="start"><SearchIcon color="warning" /></InputAdornment>) }}
              sx={{ "& .MuiOutlinedInput-root": { bgcolor: "#fff", borderRadius: 2, "& fieldset": { borderColor: "rgba(255,193,7,.6)" }, "&:hover fieldset": { borderColor: "rgba(255,193,7,.9)" }, "&.Mui-focused fieldset": { borderColor: "#ffb300" } } }}
            />
          </CardContent>
        </Card>
                        ...
                        <TableRow key={point._id || idx} hover sx={{ "&:nth-of-type(odd)": { bgcolor: "rgba(255,248,225,.35)" }, opacity: isPointActive ? 1 : 0.6 }}>
                          <TableCell>
                            <Stack direction="row" alignItems="center" spacing={1}>
                              {/* 🌟 แสดงป้าย เปิดใช้งาน/ปิดใช้งาน */}
                              <Chip size="small" color={isPointActive ? "success" : "default"} label={isPointActive ? "เปิดใช้งาน" : "ปิดใช้งาน"} sx={{ fontWeight: 800 }} />
                          ...
                          <TableCell align="center">
                            <Tooltip title="แก้ไข">
                              <span><IconButton color="primary" size="small" onClick={() => handleOpenEdit(point)} disabled={!!busyId} sx={{ mr: 0.5 }}><EditIcon /></IconButton></span>
                            </Tooltip>
                            <Tooltip title="ลบ">
                              <span>
                                <IconButton color="error" size="small" onClick={() => handleDelete(point._id)} disabled={busyId === point._id}>
                                  {busyId === point._id ? <RefreshIcon sx={{ animation: "spin 1s linear infinite" }} /> : <DeleteIcon />}
                                </IconButton>
                              </span>
                            </Tooltip>
                          </TableCell>
      ...
      <RegistrationPointDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSave={handleDialogSave} initialData={editData} isEdit={!!editData} />
